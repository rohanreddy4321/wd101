...
const dateValidity = (givenDate) => {
  ...
  const validDateYear =
    userDate[0] >= date.getFullYear() - 55 &&
    userDate[0] <= date.getFullYear() - 18;
...
submitEl.addEventListener("click", () => {
  ...
  if (!dateValidity(userDate)) {
    dobEl.setCustomValidity(
      `Date must be between ${date.getFullYear() - 55}-${digits(
        date.getMonth() + 1
      )}-${digits(date.getDate())} and ${date.getFullYear() - 18}-${digits(
        date.getMonth() + 1
      )}-${digits(date.getDate())}`
    );
  } else {
  ...
  const allValid =
    checkValidity(nameEl) &&
    checkValidity(emailEl) &&
    checkValidity(passwordEl) &&
    checkValidity(dobEl);
